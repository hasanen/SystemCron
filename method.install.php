...
if (!isset($gCms)) exit;
	...
	/*---------------------------------------------------------
	  ...
	  ---------------------------------------------------------*/
		...
		$sqlarray = $dict->CreateTableSQL(cms_db_prefix()."module_systemcron",
				$flds, $taboptarray);
		$dict->ExecuteSQLArray($sqlarray);

		// create a sequence
		$db->CreateSequence(cms_db_prefix()."module_systemcron_seq");
		*/
		
		// permissions
		$this->CreatePermission('Use system cron','Use system cron');
